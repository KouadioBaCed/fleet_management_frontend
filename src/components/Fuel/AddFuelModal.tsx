...
import type { CreateFuelData } from '@/api/fuel';

interface AddFuelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CreateFuelData) => Promise<void>;
...
export default function AddFuelModal({ isOpen, onClose, onSubmit, preselectedVehicleId }: AddFuelModalProps) {
  ...
  const [refuelDate, setRefuelDate] = useState(new Date().toISOString().slice(0, 16));
  ...
  const validate = (): boolean => {
    ...
    if (!totalCost || parseFloat(totalCost) <= 0) {
      newErrors.totalCost = 'Le coût total doit être supérieur à 0';
    }
    if (!stationName.trim()) {
      newErrors.stationName = 'Veuillez indiquer le nom de la station';
    }
    if (!mileage || parseFloat(mileage) <= 0) {
      newErrors.mileage = 'Le kilométrage doit être supérieur à 0';
    }
    if (selectedVehicle && parseFloat(mileage) < selectedVehicle.current_mileage) {
      newErrors.mileage = `Le kilométrage doit être supérieur à ${selectedVehicle.current_mileage.toLocaleString()} km`;
    }
  ...
  const handleClose = () => {
    ...
    setStationAddress('');
    setMileage('');
    setRefuelDate(new Date().toISOString().slice(0, 16));
    setIsFullTank(true);
    setNotes('');
    setManualTotal(false);
    setErrors({});
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b-2 flex items-center justify-between flex-shrink-0" style={{ borderColor: '#E8ECEC' }}>
          <div className="flex items-center space-x-3">
            ...
            >
              <Fuel className="w-5 h-5" style={{ color: '#6A8A82' }} />
            </div>
            <div>
            ...
            <div className="space-y-4">
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center space-x-2 mb-1">
                            ...
                            <span className="font-bold text-sm" style={{ color: '#191919' }}>
                              {vehicle.license_plate}
                            </span>
                          </div>
                          <p className="text-xs text-gray-500">{vehicle.brand} {vehicle.model}</p>
                          <p className="text-xs text-gray-400">{vehicle.current_mileage?.toLocaleString()} km</p>
                        </button>
                      );
                    })}
                  </div>
              ...
              {selectedVehicle && (
                <div
                  ...
                  style={{ borderColor: fuelConfig?.color, backgroundColor: fuelConfig?.bg }}
                >
                  <div className="flex items-center space-x-3 mb-3">
                    <span className="text-2xl">{fuelConfig?.icon}</span>
                    <div>
                      <p className="font-bold" style={{ color: fuelConfig?.color }}>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-medium" style={{ color: fuelConfig?.color }}>
                        {fuelConfig?.label}
                      </span>
                    </div>
                    <div>
                      <span className="text-gray-500">Capacité:</span>
                      <span className="ml-2 font-medium">{selectedVehicle.fuel_capacity}L</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Conso:</span>
                      <span className="ml-2 font-medium">{selectedVehicle.fuel_consumption}L/100km</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Km actuel:</span>
                      <span className="ml-2 font-medium">{selectedVehicle.current_mileage?.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div>
                  ...
                  {POPULAR_STATIONS.map((station) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                        stationName === station ? 'shadow-sm' : 'hover:bg-gray-100'
                      }`}
                      style={{
                ...
                <input
                  ...
                  className={`w-full px-4 py-2.5 rounded-xl border-2 text-sm focus:ring-2 focus:ring-sage/20 outline-none transition-all text-gray-900 placeholder-gray-400 ${
                    errors.stationName ? 'border-red-400' : ''
                  }`}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="0.00"
                    className={`w-full px-4 py-3 rounded-xl border-2 text-lg font-bold focus:ring-2 focus:ring-sage/20 outline-none transition-all text-gray-900 placeholder-gray-400 ${
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="0.000"
                    className={`w-full px-4 py-3 rounded-xl border-2 text-lg font-bold focus:ring-2 focus:ring-sage/20 outline-none transition-all text-gray-900 placeholder-gray-400 ${
                      errors.unitPrice ? 'border-red-400' : ''
                    }`}
              ...
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: '#191919' }}>
                  ...
                  {!manualTotal && quantity && unitPrice && (
                    <span className="text-xs text-gray-400 ml-2">(calculé automatiquement)</span>
                  )}
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    className={`w-full pl-8 pr-4 py-3 rounded-xl border-2 text-lg font-bold focus:ring-2 focus:ring-sage/20 outline-none transition-all placeholder-yellow-600 ${
                      errors.totalCost ? 'border-red-400' : ''
                    }`}
                    style={{
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder={selectedVehicle ? String(selectedVehicle.current_mileage) : '0'}
                    className={`w-full px-4 py-3 rounded-xl border-2 text-lg font-bold focus:ring-2 focus:ring-sage/20 outline-none transition-all text-gray-900 placeholder-gray-400 ${
                      errors.mileage ? 'border-red-400' : ''
                    }`}
                ...
                {selectedVehicle && !errors.mileage && (
                  <p className="text-xs text-gray-400 mt-1">
                    Dernier kilométrage: {selectedVehicle.current_mileage?.toLocaleString()} km
                  </p>
                )}
              </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {isFullTank ? 'Réservoir rempli entièrement' : 'Ravitaillement partiel'}
                    </p>
                  </div>
                </div>
                <button
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t-2 flex items-center justify-between flex-shrink-0" style={{ borderColor: '#E8ECEC', backgroundColor: '#F8FAF9' }}>
          {/* Summary */}
          <div className="flex items-center space-x-4">
            {selectedVehicle && quantity && totalCost && (
              <>
                <div className="flex items-center space-x-2">
                  <span className="text-2xl">{fuelConfig?.icon}</span>
                  <span className="font-bold">{selectedVehicle.license_plate}</span>
                </div>
                <div className="h-8 w-px bg-gray-300" />
                <div className="text-sm">
                  <span className="text-gray-500">Quantité:</span>
                  <span className="ml-1 font-bold" style={{ color: '#3B82F6' }}>{quantity}L</span>
                </div>
                <div className="h-8 w-px bg-gray-300" />
                <div className="text-sm">
                  <span className="text-gray-500">Total:</span>
                  <span className="ml-1 font-bold" style={{ color: '#B87333' }}>{currencySymbol}{totalCost}</span>
                </div>
              </>
            )}
          </div>
