...
import { X, User, CheckCircle, Clock, Coffee, XCircle, Loader2, ArrowRight, Car, AlertTriangle } from 'lucide-react';
import type { Driver } from '@/types';

interface ChangeDriverStatusModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const STATUS_OPTIONS = [
  ...
  },
  {
    value: 'on_break',
    label: 'En pause',
    description: 'Le chauffeur est temporairement indisponible',
    icon: Coffee,
    color: '#6B7280',
    bgColor: '#F3F4F6',
  },
  {
    value: 'off_duty',
    label: 'Hors service',
    description: 'Le chauffeur n\'est pas en service',
    icon: XCircle,
    color: '#DC2626',
    bgColor: '#FEE2E2',
  },
...
export default function ChangeDriverStatusModal({ isOpen, onClose, onConfirm, driver }: ChangeDriverStatusModalProps) {
  const [selectedStatus, setSelectedStatus] = useState<string | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleConfirm = async () => {
    if (!selectedStatus || selectedStatus === driver?.status) return;
  ...
  const currentStatus = getCurrentStatusInfo(driver.status);
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b-2" style={{ borderColor: '#E8ECEC' }}>
          <div className="flex items-center space-x-3">
            ...
            >
              <User className="w-6 h-6" style={{ color: currentStatus.color }} />
            </div>
            <div>
        ...
        <div className="px-6 pt-4">
          <div className="bg-gray-50 rounded-xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              {driver.photo ? (
                <img src={driver.photo} alt={driver.full_name} className="w-12 h-12 rounded-full object-cover" />
              ) : (
                <div
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{driver.total_trips} trajets</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div
              ...
              {newStatus && selectedStatus !== driver.status && (
                <>
        ...
        <div className="p-6 space-y-3">
          ...
          {STATUS_OPTIONS.map((status) => {
            ...
            const isCurrent = driver.status === status.value;
        ...
        <div className="flex items-center justify-between p-6 border-t-2 bg-gray-50" style={{ borderColor: '#E8ECEC' }}>
          ...
          <button
            ...
            disabled={isSubmitting || !selectedStatus || selectedStatus === driver.status}
